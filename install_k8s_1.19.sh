...
#Author: Anjan Dave
#Version: 1.0
printf "\n\tThis script installs kubernetes v1.19.8 on a Linux 7.x host. Make sure the CentOS install is not of a -minimum- kind, install the GNOME Desktop version and include developer, admin tools, etc"
printf "\nWe assume that latest version of Docker is installed on this host already\n"
printf "\nThis script makes a reasonable effort to check for things already installed, if you have to run it over and over again\n"
...
printf "\nFor next step we install kubernetes 1.19.8. If we have to, PLEASE MAKE SURE YOURE DISCONNECTED FROM VPN"
